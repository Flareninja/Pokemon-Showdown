...
var nightclubs = global.nightclubs;
...
function formatHTML (message) {
	...
	for (var i = 0; i < message.length; i++) {
		if (message[i] === '&') {
			var sub = message.substr(i);
			var part = sub.substr(0, sub.indexOf(';') + 1);
		...
		if (message[i] == '<') {
			var sub = message.substr(i).match(/<(span|b|a|i|code|s)[^]+<\/(span|b|a|i|code|s)>/);
			if (sub) {
...
function nightclubify (message) {
	...
	return '|raw|<div style = "font-size: 11pt; margin: -3px; padding: 1px; background: black; color: white; text-shadow: 0px 0px 10px, 0px 0px 10px, 0px 0px 10px;">' +
		...
		'<button name = "parseCommand" value = "/user ' + user + '" style = "outline: none; cursor: pointer; font-size: inherit; text-shadow: inherit; font-family: inherit; background: none; padding: 0px; border: none; display: inline-block; color: ' + Core.color(toId(user)) + ';"><b>' + user + ':</b></button>' +
...
Rooms.Room.prototype.add = function (message) {
	...
	if (this.logTimes && message.substr(0, 3) === '|c|') {
		if (nightclubs[this.id]) message = nightclubify(message.substr(3));
		else message = '|c:|' + (~~(Date.now() / 1000)) + '|' + message.substr(3);
	} else if (nightclubs[this.id]) {
		var style = '|raw|<div style = "color: white; text-shadow: 0px 0px 6px, 0px 0px 6px; background: black; padding: 2px; margin: -3px,">';
		if (message.match(/^\|html\||^\|raw\|/m) && !message.match(/\|\~\|/m)) {
			message = style + message.replace(/^\|html\||^\|raw\|/m, '') + '</div>';
		}
		else if (!message.match(/^\|[a-z]+\|/) && !message.match(/^\|\|/)) message = style + message + '</div>';
	}
	...
	if (roomid && nightclubs[roomid.id]) {
		...
		if (data.match(/^\|html\||^\|raw\|/m) && !data.match(/\|\~\|/m)) {
			if (data.split('\n').length > 1) {
				...
				data = parts.join('\n');
			} else data = style + data.replace(/^\|html\||^\|raw\|/m, '') + '</div>';
		}
		else if (!data.match(/^\|[a-z]+\|/) && !data.match(/^\|\|/)) data = style + data + '</div>';
	}
